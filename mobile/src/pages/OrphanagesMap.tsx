...
export default function OrphanagesMap(){
  const [orphanages, setOrphanages] = useState<Orphanage[]>([]); //lista se inicia com useState([]) <Orphanage[]> --> armazena array do formado Orphanage
  ...
  return(
    <View style={styles.container}>
      <MapView 
        ...
        style={styles.map} 
        initialRegion={{latitude: -22.9973335, longitude: -47.5147301, latitudeDelta: 0.008, longitudeDelta: 0.008,}} 
      >
        {orphanages.map(orphanage => {
          return(
            <Marker
              key={orphanage.id} //campo unico
              icon={mapMarker}
              calloutAnchor={{
                x: 2.7,
                y: 0.9,
